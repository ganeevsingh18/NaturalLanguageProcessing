...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(embed_size, embed_size, kernel_size=2, padding='same').to(self.device)
        self.encoder = nn.LSTM(embed_size, hidden_size, bias=True, dropout=dropout_rate, bidirectional=True).to(self.device)
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size, bias=True).to(self.device)

        self.h_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False).to(self.device)
        self.c_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False).to(self.device)
        self.att_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False).to(self.device)

        self.combined_output_projection = nn.Linear(hidden_size * 3, hidden_size, bias=False).to(self.device)
        self.target_vocab_projection = nn.Linear(hidden_size, len(vocab.tgt)).to(self.device)

        self.dropout = nn.Dropout(p=dropout_rate).to(self.device)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        #adding model embeddings
        # print(f'SOurce padded shape: {source_padded.shape}')
        X = self.model_embeddings.source(source_padded) #( src_len, b, e)
        # print(X.shape)

        #passing through conv layer
        X=X.permute(1,2,0) # ( b, e, src_len)

        X = self.post_embed_cnn(X)
        X= X.permute(2,0,1) # (src_lem , b , e)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1 , dim=0):
            ...
            Y_t=Y_t.squeeze(0) #squeezing into (b,e)
